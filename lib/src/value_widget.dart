...
/// }
/// ```
abstract class ValueWidget<TValue> extends StatefulWidget {
  /// You can limit the rebuild range by tucking it inside a widget.
  ///
  /// Basically, it is used in the same way as [ConsumerWidget].
  ///
  ...
  @override
  @protected
  State<StatefulWidget> createState() => ValueWidgetState<TValue>();
}

class ValueWidgetState<TValue> extends State<ValueWidget<TValue>> {
  /// Describes the part of the user interface represented by this widget.
  ///
  ...
  @override
  Widget build(BuildContext context) {
    final value = ValueProvider._of<TValue>(context);
    return Scoped(
...
/// Notifier to convey a specific value to the widget directly below it.
///
/// Only [ValueWidget<TValue>] can be placed under this.
///
/// For [value], specify the value you want to pass.
class ValueProvider<TValue> extends InheritedWidget {
  ...
  const ValueProvider({
    Key? key,
    required this.value,
    required ValueWidget<TValue> child,
